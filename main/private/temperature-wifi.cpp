...
void event_handler(void *handler_arg, esp_event_base_t base, int32_t id, void *event_data)
{
  ...
  switch (id)
  {
    ...
    {
      ESP_LOGI(TAG, "Station discontected");
      temperature_wifi_ref->retry_connect();
      break;
    }
    ...
    default:
    {
      ESP_LOGW(TAG, "Unexpected event id: %li", id);
      break;
    }
  }
}

Temperature_wifi::Temperature_wifi(wifi_config_t *config, Temperature_status *status)
{
  this->config = config;
...
esp_err_t Temperature_wifi::create_event_loop()
{
  ...
  esp_err_t return_code = esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &event_handler, this);
...
void Temperature_wifi::retry_connect()
{
